...
class AddMeshIntoSceneCallBack : public Callback
{
public:
	void execute(const Params& params) override
	{
		const MeshParams& castedCmdParams = static_cast<const MeshParams&>(params);
		Mesh* mesh = castedCmdParams.m_mesh;
		...
		if (!sceneContainsMesh)
		{
			...
			addMeshFacesIntoOctree(castedCmdParams);
		}
