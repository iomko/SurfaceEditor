...
public:
	void execute() override
	{
		...
		std::vector<Mesh*>& selectedMeshes = ViewPortsHolderContext::m_objectSelectionHolder->m_meshes;
		...
		while(!selectedMeshes.empty())
		{
			...
			MeshSelectionManager::unregisterMesh(*selectionsHolder, mesh);
			//---DELETE_FROM_SELECTIONS---
		}
	}
...
private:
	...
	{
		for (HalfEdgeDS::Face* face : mesh->m_halfEdgeStructure->m_faces)
		{
	...
	void deleteMeshMeshData(Mesh* mesh)
	{
		...
		{
			HalfEdgeDS::Face* face = mesh->m_halfEdgeStructure->m_faces.back();
