#include "BasicSculptToolCommand.h"
#include "CommandRegistry.h"
#include <glm/glm.hpp>

static AutoRegister<BasicSculptToolCommand> regAddCube("BasicSculptTool");

void BasicSculptToolCommand::execute(){}
void BasicSculptToolCommand::undo(){}

glm::vec3 BasicSculptToolCommand::computeAverageNormal(const std::vector<glm::vec3> &normals)
{
    glm::vec3 sum(0.0f);
...
std::pair<bool, float> BasicSculptToolCommand::isPointInsideSphere(const glm::vec3 &point, const glm::vec3 &sphereCenter, float sphereRadius)
{
...
float BasicSculptToolCommand::logarithmicInterpolation(float edge0, float edge1, float x)
{
    x = std::max(0.0f, std::min((x - edge0) / (edge1 - edge0), 1.0f));
    return log(x * (exp(1) - 1) + 1) / log(exp(1));
}

float BasicSculptToolCommand::smoothstep(float edge0, float edge1, float x)
{
    x = std::max(0.0f, std::min((x - edge0) / (edge1 - edge0), 1.0f));
    return x * x * (3 - 2 * x);
}

float BasicSculptToolCommand::calculateBrushScalingFactor(float distance, float radius, float brushStrength)
{
...
std::pair<std::vector<ExtendedVertex *>, std::vector<ExtendedFace *>> BasicSculptToolCommand::accessNeighbouringVertsAndFacesOfVertex(ExtendedVertex *vertex)
{
