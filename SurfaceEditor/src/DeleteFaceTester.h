...
public:
	template <typename T>
	static void swapWithLastAndPop(std::vector<T>& vector, size_t index) {
	...
	template <typename T>
	static void reverseSubrange(std::vector<T>& vec, size_t startingIndex, size_t endingIndex) {
	...
	static void deleteFaceMeshData(Mesh* mesh, HalfEdgeDS::Face* face)
	{
